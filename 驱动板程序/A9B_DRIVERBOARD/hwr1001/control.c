...
void System_Control (void)
{
	...
	if (bsp_init_flag == 1) //等待底层初始化完成
	{
		...
		{
			//if ((can_Receive_Right_flag == 1) && (driver_over_current_flag == 0) && (soft_reset_flag ==0)) //驱动板使能,接收数据正确         //不再对过流进行检测 YanBing
                        if ((can_Receive_Right_flag == 1) && (soft_reset_flag ==0)) //驱动板使能,接收数据正确
			{
