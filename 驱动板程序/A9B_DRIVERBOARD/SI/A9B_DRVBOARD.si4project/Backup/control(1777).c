...
void Eep_Data_Update ( void )
{
	...
	if (uart1_Receive_Right_flag == 1) //接收到正确数据
	{
		...
		//printf ("\r\n ID:0x%x 电流：%2.3fA 细分：%d ", ID, current, step);
		//printf ("\r\n 参数更新成功......");
...
void CAN_SendData_Acquisition (void)
{
	...
	DRIVER_TO_MAIN_DATA[3] = Location_Cnt_tmp >> 24;
	DRIVER_TO_MAIN_DATA[4] = (Location_Cnt_tmp >> 16) & 0x00ff;
	DRIVER_TO_MAIN_DATA[5] = (Location_Cnt_tmp >> 8) & 0x0000ff;
	DRIVER_TO_MAIN_DATA[6] = Location_Cnt_tmp  &0x000000ff;
...
void Check_Location (void)
{
	...
	if(abs(gCurrent_pos) >= abs(CaptureNumber)) //正方向
	{
		...
		{
			if(abs(abs(gCurrent_pos) - abs(CaptureNumber)) <= 1)
			{
				gCurrent_pos = gNew_pos;
			}
		}
	...
	//if (CaptureNumber != (s32)(Aim_Location))
	if(CaptureNumber != (s32)(gCurrent_pos))
	{
